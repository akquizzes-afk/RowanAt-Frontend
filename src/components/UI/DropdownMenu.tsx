...
export const DropdownMenu = ({ isOpen, onClose }: DropdownMenuProps) => {
  ...
  const menuItems = [
    ...
    { icon: <Gift className="w-5 h-5" />, label: 'Share & Get Credits', color: 'text-emerald-400' }, // Fixed text
      ...
      >
        <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-800 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-4 border-b border-gray-800 flex items-center justify-between">
            <h3 className="font-medium text-gray-200">Quick Menu</h3>
            <button
              onClick={onClose}
          ...
          <div className="p-2 space-y-1">
            {menuItems.map((item, index) => (
              <button
                key={index}
                className="w-full flex items-center justify-between p-3 rounded-xl hover:bg-gray-800/50 transition-all duration-200 group active:scale-[0.98]"
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg bg-gray-800/50 ${item.color} flex-shrink-0`}>
                    {item.icon}
                  </div>
                  <span className="text-gray-300 font-medium text-sm text-left group-hover:text-white transition-colors">
                    {item.label}
                  </span>
                </div>
